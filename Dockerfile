...
RUN export DEBIAN_FRONTEND=noninteractive \
	...
	\
	&& apt-get -y install netbase libssl-dev \
	...
	&& cd dbpager && ./autogen.sh && ./configure --disable-dbp_cgi --disable-mod_dbp --disable-dbp_odbc --disable-dbp_isapi --disable-dbp_gearman --disable-dbp_mongo --disable-dbp_script && make -j4 && make install && cd .. \
	\
...
RUN export DEBIAN_FRONTEND=noninteractive \
	...
	&& echo "deb http://security.debian.org buster/updates main contrib non-free" >> /etc/apt/sources.list \
	&& apt-get update && apt-get -y dist-upgrade \
	&& apt-get -y install libssl1.1 libcurl4 libjsoncpp1 libpcre16-3 libpcre32-3 libpcrecpp0v5 libxml2 libsqlite3-0 libmemcached11 libmemcachedutil2 libxslt1.1 libdb5.3++ libpqxx-6.2 libhiredis0.14 libmosquittopp1 \
	&& apt-get -y clean \
	&& rm -rf /tmp/* \
	&& rm -rf /var/tmp/* \
